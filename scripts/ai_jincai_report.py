...
from build_odds_store import odds_store_context
...
ODDS_SEARCH_SITE_QUERIES = [
    ...
    "site:odds.zgzcw.com 即时赔率 凯利指数 亚盘 半全场",
    "site:live.titan007.com 足球 欧赔 亚盘 半全场",
...
def build_search_supplement(*, day: str, source_blocks: str, mode: str) -> str:
    ...
    base = f"{day} 竞彩足球 今日 欧赔 亚盘 半全场 凯利 必发 伤停 战意 竞彩网 500彩票网 足彩网 澳客 7M 新球体育"
    ...
    for match_id, league, home, away in extract_jincai_matches(source_blocks):
        if mode == "plan":
            queries.append(
                ...
                "欧赔 亚盘 半全场 凯利 必发 竞彩网 500彩票网 足彩网 澳客 7M 新球体育"
            )
...
3. 如果无法从片段中可靠提取最新赛程、玩法、赔率、凯利、亚盘、半全场、伤停或战意，必须明确写数据缺口；不得编造。大小球只作为盘口背景或结构证据记录，不得输出为预测主项。
...
11. 缺少亚盘、凯利、伤停、首发或战意中的部分数据时，应增加不确定性惩罚，不得机械地把全部场次标红。首发不可见是常见赛前缺口，不是执行绿灯的一票否决项；只要赔率方向、盘口支撑和至少一类可靠外部确认足够同向，可以给执行绿。存在方向矛盾、多个未覆盖核心风险或全部外部确认缺失时，仍不得升级为执行绿灯。
12. 当日竞彩场次达到 8 场及以上时，必须输出最多 3 场观察绿灯，或逐场说明为何没有任何场次达到观察绿灯。不得为了数量硬升绿。
13. 必须加入“热门穿盘观察”独立分支：逐场检查让球胜是否有可复核价值。至少记录具体让球胜玩法、胜平负与让球胜跨窗口变化、亚盘是否退盘、半全场或比分路径是否支持、外部确认、数据缺口、A/B/C档、三分、最大反方证据和推翻条件。前10场只进入模拟池，不进入正式推荐或串关。
14. 部分竞彩比赛只开放让球胜平负，不开放胜平负。此时必须把让球玩法作为独立锁版场次分析，不得把“胜平负未开售”误写成比赛缺失，也不得虚构胜平负赔率。
15. 半全场是独立预测玩法；竞彩总进球和亚洲大小球仅作辅助背景，不进入预测主项、黄绿灯主计算或正式收益口径。不得把大小球/总进球当作半全场证据，也不得把半全场赔率写成大小球结论。
16. 必须加入“历史相似盘口观察”独立分支：只使用赛前快照，亚盘、半全场、欧赔和竞彩玩法分栏记录；输出样本数、全网/同联赛样本、亚盘赢走输、半全场命中/失败、欧赔胜平负和数据缺口。滚球数据不得混入赛前分母；样本不足20场时只记录，不得补造匹配结果或写100%概率，不得单独升级执行绿。
17. 必须加入“半全场变化与预测”独立主表。11:30 记录半全场赔率基线；17:00 比较 11:30 与当前窗口；21:05 比较 11:30、17:00 与当前窗口。逐场写清具体半全场选项（胜胜、平胜、平平等）、代表来源、初始/当前或跨窗口赔率、去水概率、变化信号、半全场预测（具体方向或观察）、置信等级、核心依据、最大反方证据和推翻条件。缺少可靠赔率时写明缺口，不得补造。
18. 半全场预测只作为独立观察项和其他方向的结构证据，不得因为单一半全场信号把黄灯或红灯升级为执行绿。若当前只有一个快照，明确写“仅建立基线，暂无跨窗口变化结论”。大小球不输出预测主表；如抓到大小球，仅写入辅助背景/数据缺口，不计入黄绿灯。
...
20. 必须加入“世界杯/国家队冷门观察”独立分支：仅适用于世界杯、世预赛、洲际杯、国家队友谊赛等国家队场景。赔率、亚盘、凯利、半全场和跨窗口变化是主线；大名单、伤停、停赛、预计首发、长途旅行、战意和轮换只作为确认、降级或推翻条件，不得用基本面故事单独升绿，也不得因为没有首发就机械压低所有国家队场次。
21. 国家队冷门观察必须逐场写清：赛事属性、强队名气热度、受让方保护方向、胜平负双选/让球双选方向、大名单核验状态、核心缺口位置（门将/中卫/后腰/锋线）、赔率是否已经消化基本面、最大反方证据、推翻条件和执行层级。名单无法可靠核验时必须写“名单缺口”，并增加不确定性惩罚；但名单或首发缺口不能单独否决赔率和盘口已经充分共振的方向。
22. 世界杯/国家队场景不得直接套用五大联赛冷门率。强队深让、名气过热、战意不透明、轮换未知时最多升观察绿；如果赔率结构、盘口水位、半全场/凯利/跨源欧赔至少两类同向，且没有确认伤停或战意反向，可以不等首发进入执行绿候选。
23. 必须使用“结构化赔率方向摘要”辅助判断跨窗口共振：亚盘让球、半全场、竞彩让球指数和欧赔分开看；亚洲大小球只作辅助背景。只有一个快照时只能写基线；两个以上快照才写变化。无法解析的 raw_only 行只能作为数据证据，不能补造方向。
...
3. 复盘要区分正式方案、模拟方案、黄灯双选/让球双选观察、热门穿盘观察、红灯跳过。
...
7. 历史相似盘口观察必须单独复盘：记录赛前样本数、全网/同联赛分栏、亚盘赢走输、半全场命中/失败、欧赔胜平负、ROI与校准缺口。滚球样本不得混入；样本门槛未满足时不得调整主模型权重。
...
10. 首发不可见不是赛前错误的充分条件。复盘时只有确认首发/轮换与赛前方向相反并实际影响结果，才记录为阵容误读；否则归入“常规信息缺口”，不得把所有失败都归因于未看到首发。
11. 复盘必须检查结构化赔率方向摘要是否支持赛前判断：亚盘、半全场、竞彩让球指数、欧赔分开验收；大小球仅作为辅助背景，无法解析的 raw_only 行只记录为数据质量问题。
